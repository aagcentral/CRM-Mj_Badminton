    ...
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-md-6 col-sm-12">
                <h4>Create And Manage Payment Module</h4>
            </div>
            <div class="col-md-6 col-sm-12">
                <ol class="breadcrumb float-sm-right" style="font-family: sans-serif;">
                    ...
                    <li class="breadcrumb-item active">Payment Module</li>
                </ol>
            </div>
        </div>
    </div>
...
<div class="container-fluid py-3">
    <div class="row mt-4">
        <div class="col-md-12 d-flex">
            @if(havePermission('paymentmodule.add'))
            <button id="toggleButton" class="btn btn-info mb-3 ml-1"><i class="fas fa-plus-circle me-2"></i>Add Payment Module</button>
            @endif
        </div>
        @if ($errors->any())
    ...
    <div class="card togglediv overflow-auto" @if (!$errors->any()) style="display: none;" @endif>
        ...
        <div class="card-body">
            <div class="row">
                <form action="{{route('paymentmodule.add')}}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label for="name">Add Payment Module <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="module" value="{{ old('module') }}" placeholder="Enter Payment Module">
                            </div>
                        </div>
                ...
                <tbody>
                    ...
                    <tr class="">
                        ...
                        <td>
                            ...
                            @if(havePermission('paymentmodule.edit'))
                            <a href="{{ route('paymentmodule.edit', $row->id) }}" class="text-info px-2"><i class="fas fa-edit"></i> </a>
                            @endif
                            @if(havePermission('paymentmodule.destroy'))
                            <button class="btn btn-default text-danger btn-sm px-2 delete-paymentmodule" data-id="{{ $row->id }}"><i class="fa-solid fa-trash"></i> </button>
                            @endif
                        </td>
...
<script>
    $('.delete-paymentmodule').click(function() {
        var id = $(this).data('id');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('paymentmodule.destroy') }}",
                    type: 'POST',
                    data: {
                        id: id,
                        _token: "{{ csrf_token() }}"
